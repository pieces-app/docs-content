...
metaTitle: Chat with Conversational Search in JetBrains | Pieces Docs
...
## Accessing Conversational Search Chat in JetBrains IDEs

There are several ways to open up Conversational Search chat window in any JetBrains IDEs.
...
<Steps>
  ...
  <Step title="Query Conversational Search">
    Type **"Conversational Search"** and select it from the search results to open the chat
...
You can utilize **2 different levels of contextual awareness** when initiating a Copilot chat (or adding code to an existing chat) with Conversational Search.
...
Once you've entered your question, Conversational Search will analyze the file's context to deliver precise responses.
...
This can be done without opening Conversational Search window. Right-click on a file in your open project and select `Add to Copilot Chat Context`.
...
Conversational Search will then scan the screenshot and generate the code captured from the image into the chat. You can copy, insert it at your cursor, save it as a snippet, and more.
...
Click `Pieces: Explain` to open up Conversational Search in the side window. The Conversational Search will automatically explain the purpose and function of that code within the chat.
...
The Conversational Search Chat is designed to assist you with various coding tasks to boost productivity and enhance your workflow. This is done primarily by eliminating **context-switching** (needing to leave your IDE to access generative AI).

### Naming Inconsistencies

Suppose functions across your workspace use inconsistent naming patterns (e.g., `authenticateUser` in `authHandler.go` vs. `retrieveUserProfile` in `userHandler.go`). In that case, Conversational Search can suggest adopting a standardized naming convention for better readability and maintainability, like this:
