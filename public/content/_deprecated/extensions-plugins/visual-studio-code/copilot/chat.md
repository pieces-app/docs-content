...
metaTitle: Chat with Conversational Search in VS Code | Pieces Docs
...
## Conversational Search — in your IDE

Conversational Search brings the power and flexibility of local and cloud-hosted LLMs together with the context-aware Long-Term Memory Engine for intelligent & aware conversations, code documentation, smart debugging, and more.

## Accessing Copilot Chat in VS Code

There are three ways to primary ways to access generative AI chats with Conversational Search.
...
You can also open up a dialogue with Conversational Search—specifically the `Ask Copilot About Selection` option—by using the shortcut `⌘+shift+a` (macOS) or `ctrl+shift+a` (Windows/Linux).
...
There are 3 different levels of contextual awareness that you can utilize when initiating a Copilot chat (or adding code to an existing chat) with Conversational Search.
...
With the `Pieces: Ask Copilot About Workspace` command, you can extend Conversational Search’s assistance to fit the project scope.
...
Conversational Search will then scan the screenshot and generate the code captured from the image into the chat, from which you can copy, insert at your cursor, save it as a snippet, and more.
